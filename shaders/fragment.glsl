...
vec3 getBlendedColorAtPoint(float distToViewer, vec3 directionToViewer) {
    ...
    if (blendFactor > 0){
      vec3 nextColor = mix(getBlendedColorForHorizontal(min(flooredTextureGroupIdx + 1, 4), fragPos.x, distToViewer, directionToViewer),
                           getBlendedColorForHorizontal(flooredTextureGroupIdx + 1, fragPos.z, distToViewer, directionToViewer), 0.5);
